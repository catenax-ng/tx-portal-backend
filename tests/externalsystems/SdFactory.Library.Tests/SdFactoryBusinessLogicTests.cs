...
public class SdFactoryBusinessLogicTests
{
    ...
    public async Task ProcessFinishSelfDescriptionLp_ConfirmWithValidData_CompanyIsUpdated()
    {
        ...
        A.CallTo(() => _documentRepository.CreateDocument($"SelfDescription_LegalPerson.json", A<byte[]>._, A<byte[]>._, "application/json", DocumentTypeId.SELF_DESCRIPTION, A<Action<Document>?>._)).MustHaveHappenedOnceExactly();
    ...
    public async Task ProcessFinishSelfDescriptionLp_FailedWithValidData_CompanyIsUpdated()
    {
        ...
        A.CallTo(() => _documentRepository.CreateDocument("SelfDescription_LegalPerson.json", A<byte[]>._, A<byte[]>._, "application/json", DocumentTypeId.SELF_DESCRIPTION, A<Action<Document>?>._)).MustNotHaveHappened();
    ...
    public async Task ProcessFinishSelfDescriptionLpForConnector_ConfirmWithValidData_CompanyIsUpdated()
    {
        ...
        // Assert
        A.CallTo(() => _documentRepository.CreateDocument("SelfDescription_Connector.json", A<byte[]>._, A<byte[]>._, A<string>._, DocumentTypeId.SELF_DESCRIPTION, A<Action<Document>?>._)).MustHaveHappenedOnceExactly();
    ...
    private void SetupForProcessFinish(Company company, ApplicationChecklistEntry applicationChecklistEntry)
    {
        ...
        A.CallTo(() =>
                _documentRepository.CreateDocument(A<string>._, A<byte[]>._, A<byte[]>._, A<string>._, A<DocumentTypeId>._,
                    A<Action<Document>?>._))
            .Invokes((string documentName, byte[] documentContent, byte[] hash, string mimeType, DocumentTypeId documentTypeId, Action<Document>? setupOptionalFields) =>
            {
                var document = new Document(documentId, documentContent, hash, documentName, mimeType, DateTimeOffset.UtcNow, DocumentStatusId.PENDING, documentTypeId);
                setupOptionalFields?.Invoke(document);
                _documents.Add(document);
            })
            .Returns(new Document(documentId, null!, null!, null!, null!, default, default, default));
    ...
    private void SetupForProcessFinishForConnector(Connector connector)
    {
        ...
        var documentId = Guid.NewGuid();
        A.CallTo(() =>
                _documentRepository.CreateDocument(A<string>._, A<byte[]>._, A<byte[]>._, A<string>._, A<DocumentTypeId>._, A<Action<Document>?>._))
            .Invokes((string documentName, byte[] documentContent, byte[] hash, string mimeType, DocumentTypeId documentTypeId, Action<Document>? setupOptionalFields) =>
            {
                var document = new Document(documentId, documentContent, hash, documentName, mimeType, DateTimeOffset.UtcNow, DocumentStatusId.PENDING, documentTypeId);
                setupOptionalFields?.Invoke(document);
