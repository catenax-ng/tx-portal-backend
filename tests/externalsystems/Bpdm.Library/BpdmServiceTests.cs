...
public class BpdmServiceTests
{
    ...
    public async Task FetchInputLegalEntity_WithValidResult_ReturnsExpected()
    {
        ...
        // Act
        var result = await sut.FetchInputLegalEntity(Guid.NewGuid(), CancellationToken.None).ConfigureAwait(false);
    ...
    public async Task FetchInputLegalEntity_WithEmtpyObjectResult_ThrowsServiceException()
    {
        ...
        // Act
        async Task Act() => await sut.FetchInputLegalEntity(Guid.NewGuid(), CancellationToken.None).ConfigureAwait(false);
