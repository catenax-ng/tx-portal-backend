...
{
    IAsyncEnumerable<bool> ExecuteProcess(Guid processId);
}

public interface IProcessTypeExecutorFactory
{
    IProcessTypeExecutor GetInstance(ProcessTypeId processTypeId);
}
...
public interface IProcessTypeExecutor
{
    ...
    Task<InitializationResult> InitializeProcess(Guid processId);
    Task<StepExecutionResult> ExecuteProcessStep(ProcessStepTypeId ProcessStepTypeId, IEnumerable<ProcessStepTypeId> ProcessStepTypeIds);
