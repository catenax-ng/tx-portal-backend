...
public class ApplicationActivationService : IApplicationActivationService
{
    ...
    private async Task<(Action<ApplicationChecklistEntry>?,IEnumerable<ProcessStepTypeId>?,bool,IEnumerable<ProcessStepTypeId>?)> HandleApplicationActivationInternal(IChecklistService.WorkerChecklistProcessStepData context)
    {
        ...
        return (entry => entry.ApplicationChecklistEntryStatusId = ApplicationChecklistEntryStatusId.DONE, null, true, Enum.GetValues<ProcessStepTypeId>().Except(new [] { ProcessStepTypeId.ACTIVATE_APPLICATION }));
    }
