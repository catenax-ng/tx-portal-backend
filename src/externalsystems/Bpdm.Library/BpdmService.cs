...
public class BpdmService : IBpdmService
{
    ...
    public async Task<bool> PutInputLegalEntity(BpdmTransferData data, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                new(
                    data.CompanyId.ToString(),     // ExternalId
                    null,                          // Bpn
    ...
    public async Task<BpdmLegalEntityData?> FetchInputLegalEntity(Guid companyId, CancellationToken cancellationToken)
    {
        var httpClient = await _tokenService.GetAuthorizedClient<BpdmService>(_settings, cancellationToken).ConfigureAwait(false);
        var result = await httpClient.GetAsync($"api/catena/legal-entities/{companyId}", cancellationToken).ConfigureAwait(false);
        if (!result.IsSuccessStatusCode)
        {
